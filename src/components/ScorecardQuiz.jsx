  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const templateParams = {
        ...
        message_html: `
          <h2>Your AI Readiness Score: ${percentage}%</h2>
          <p><strong>Level:</strong> ${recommendations.level}</p>
          <p>${recommendations.description}</p>
          <h3>Recommended Next Steps:</h3>
          <ul>
            ${recommendations.actions.map(action => `<li>${action}</li>`).join('')}
          </ul>
          <p>Schedule a consultation with our team to discuss your AI transformation journey.</p>
        `
      };
